...
class TradingBot:
                ...
                async with session.get(
                    ...
                    async with session.post(
                        ...
                        if action == TradeAction.BUY:
                            ...
                            self.current_position = None
                            self.position_open_time = None
                            # Activate cooldown after successful sell
                            self.cooldown_end_time = time.time() + 25
    ...
    async def monitor_token(self, token_address: str):
        """
        Monitor the position with both a fixed stop loss and a trailing stop loss.
        
        - A fixed stop loss is in place (e.g., -5% from the buy price) to exit early if the price drops too far before reaching the profit target.
        - Once the profit target (20%) is reached, the trailing stop loss activates.
          It exits if the current price falls by approximately 1.5% from the highest price reached after activation.
        """
        profit_target_pct = 20         # Profit target to activate trailing stop
        trailing_stop_pct = 1.5        # Trailing stop threshold (~1.5% drop triggers exit after profit target)
        fixed_stop_loss_pct = -55       # Fixed stop loss if price falls 5% below the buy price (before profit target)
        trailing_active = False        # Flag to indicate if trailing stop is active
        highest_price = self.buy_price  # Initialize highest price with the buy price
        ...
        while self.current_position == token_address:
            try:
                ...
                # Calculate profit percentage relative to buy price
                profit_pct = ((current_price - self.buy_price) / self.buy_price) * 100

                # If trailing stop isn't active, check for fixed stop loss first.
                if not trailing_active and profit_pct <= fixed_stop_loss_pct:
                    self.logger.info(f"Fixed stop loss triggered at {profit_pct:.2f}% profit.")
                    retry_count = 0
                    while retry_count < 3 and self.current_position:
                        success = await self.execute_trade(token_address, TradeAction.SELL)
                        if success:
                ...
                # Activate trailing stop when profit target is reached.
                if not trailing_active:
                    if profit_pct >= profit_target_pct:
                        trailing_active = True
                        highest_price = current_price
                        self.logger.info(f"Profit target reached ({profit_pct:.2f}%). Trailing stop activated.")
                else:
                    # Update highest price reached after trailing stop activation.
                    if current_price > highest_price:
                        highest_price = current_price
                        self.logger.info(f"New highest price reached: ${highest_price:.6f}")

                    # Calculate trailing stop price threshold.
                    trailing_stop_price = highest_price * (1 - trailing_stop_pct / 100)
                    if current_price < trailing_stop_price:
                        self.logger.info(
                            f"Trailing stop triggered. Current price ${current_price:.6f} fell below trailing stop price ${trailing_stop_price:.6f}."
                        )
                        retry_count = 0
                        while retry_count < 3 and self.current_position:
                            success = await self.execute_trade(token_address, TradeAction.SELL)
                ...
                # Optionally, exit if the position is held for too long (e.g., 1 hour)
                if time.time() - self.position_open_time > 7200:
                    self.logger.info("1 hour hold timeout triggered.")
                    retry_count = 0
                    while retry_count < 3 and self.current_position:
                        success = await self.execute_trade(token_address, TradeAction.SELL)
                        if success:
